function qcaudio(listdata){
	// Settings
	var repeat = localStorage.repeat || 0, //循环参数
		shuffle = localStorage.shuffle || 'false', // 随机参数
		continous = true,  
		autoplay = true,
		playlist =listdata;
	...
	var time = new Date(),
		currentTrack = shuffle === 'true' ? time.getTime() % playlist.length : 0, //随机歌曲当前的参数
		trigger = false,
		audio, timeout, isPlaying, playCounts;  //参数设置
...
//   点击播放音乐事件的功能
	...
	$('.mobile-tuwen-bf').each(function(){
	   ...
	   //if (isPlaying == true) play();
		...
		newaudio = $('<audio>').html('<source src="'+jsoncont.realFileURL+'"><source src="'+jsoncont.ogg+'">').appendTo('#player');
		$('.cover').html('<img src="'+jsoncont.realImgURL+'" alt="'+jsoncont.album+'">');
		$('.tag').html('<strong>'+jsoncont.title+'</strong><span class="artist">'+jsoncont.artist+'</span><span class="album">'+jsoncont.album+'</span>');
		audio = newaudio[0];
		audio.volume = $('.mute').hasClass('enable') ? 0 : volume;
		audio.addEventListener('progress', beforeLoad, false);
		audio.addEventListener('durationchange', beforeLoad, false);
